...
const ChatView = ({ 
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] bg-white/40 backdrop-blur-md rounded-[2.5rem] border border-primary/5 overflow-hidden shadow-sm">
      {/* Left Panel: Contacts List */}
      <div className={`${activeChatId ? 'hidden md:flex' : 'flex'} w-full md:w-80 border-r border-primary/5 flex-col bg-white/20 h-full overflow-hidden`}>
        <div className="p-6 md:p-8 border-b border-primary/5 shrink-0">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl md:text-2xl font-serif font-bold italic text-primary">Messages</h3>
            <span className="bg-primary/5 text-primary text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-widest">{filteredChats.length}</span>
          </div>
          <div className="relative">
            <input 
              type="text" 
            ...
            />
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-primary/30" size={16} />
          </div>
        </div>
            ...
            filteredChats.map(chat => (
              ...
              >
                {activeChatId === chat.tutorId && (
                  <motion.div layoutId="active-chat" className="absolute left-0 top-2 bottom-2 w-1 bg-primary rounded-r-full" />
                )}
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h5 className={`text-sm truncate transition-colors ${activeChatId === chat.tutorId ? 'font-bold text-primary text-base' : 'font-semibold text-primary/70'}`}>
                      {chat.tutorName}
                    </h5>
                    <span className="text-[10px] font-bold text-primary/40 uppercase tracking-widest shrink-0 ml-2">{chat.time}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className={`text-xs truncate font-medium ${chat.unreadCount > 0 ? 'text-black' : 'text-black/40'}`}>{chat.lastMessage}</p>
                    {chat.unreadCount > 0 && (
                      <span className="bg-primary text-white text-[9px] font-black min-w-[18px] h-[18px] flex items-center justify-center rounded-full shrink-0 ml-2 shadow-sm">
                        {chat.unreadCount}
                      </span>
                    )}
                  </div>
      ...
      <div className="p-4 md:p-6 bg-white/60 backdrop-blur-md border-b border-primary/5 flex items-center justify-between px-6 md:px-10 z-10">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <button 
            onClick={() => setActiveChatId(null)}
            className="md:hidden p-2 hover:bg-primary/5 rounded-xl text-primary/60 transition-all"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="relative">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p className="text-[8px] md:text-[10px] font-bold text-emerald-600 uppercase tracking-widest opacity-80">Online Now</p>
            </div>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-6 md:space-y-8 custom-scrollbar bg-pattern-dots">
              {activeChat.messages.map((msg, idx) => {
                ...
                const msgDate = (msg as any).date || 'TODAY';
                const prevDate = idx > 0 ? ((activeChat.messages[idx - 1] as any).date || 'TODAY') : null;
                const showDateSep = idx === 0 || msgDate !== prevDate;
                return (
                  <React.Fragment key={msg.id}>
                    {showDateSep && (
                      <div className="flex justify-center my-2 md:my-4">
                        <span className="bg-slate-200/80 text-slate-500 text-[9px] md:text-[10px] font-bold px-4 py-1 rounded-full uppercase tracking-widest shadow-sm">
                          {msgDate}
                        </span>
                      </div>
                    )}
